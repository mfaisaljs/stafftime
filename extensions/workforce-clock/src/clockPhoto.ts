export type CapturedPhoto = {
  /** Raw base64 only — server adds the data URL prefix. */
  photo: string;
  photoType: string;
  previewSrc: string;
};
...
/** Opens the camera until the capture differs from the stored clock-in selfie. */
export async function captureClockOutSelfie(
  ...
  // POS camera APIs often return the previous capture unless given time to reset.
  await delay(700);
